...
{
    server_ctx_t *ret = (server_ctx_t*)malloc(sizeof(server_ctx_t));
    ret->ctx = s;
    ret->port = malloc(sizeof(char) * strlen(port));
    strcpy(ret->port, port);
    return ret;
}
...
client_ctx_t *init_client_ctx(shared_ctx_t *s, power_ctx_t *power_ctx, char **hosts, char *host, int len, time_t runtime, int interval_msecs)
{
    ...
    int secs = (int)(interval_msecs / 1000);
    int nanosecs = (interval_msecs % 1000) * 1000000;
    ret->interval.tv_sec = secs;
    ret->interval.tv_nsec = nanosecs;
    return ret;
}
