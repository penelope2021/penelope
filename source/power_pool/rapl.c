...
power_ctx_t *init_power_ctx(int len)
{
    ...
    power_ctx_t *data = malloc(sizeof(power_ctx_t));
    data->len = len;
    data->readings = malloc(sizeof(*data->readings) * len);
    memset(data->readings, 0, len * sizeof(*data->readings));
    return data;
}
...
{
    power_ctx_t *ctx = (power_ctx_t *)args;
